...
void main() {
  group('Payment flow integration', () {
    test('normalizedText produces consistent inputHash for duplicate detection', () {
...
[19/05/2024, 10:34] Alice: how are you?''';
      ...
      expect(hash1.length, 16, reason: 'Truncated SHA-256 produces 16 hex chars');
    });

    test('token estimation flows through import → setup → payment correctly', () {
...
[19/05/2024, 10:34] Alice: I am doing well thanks for asking''';
    ...
    test('chunking decision is correct for small chat', () {
      ...
      final chunks = TokenCalculator.splitIntoChunks(smallChat, maxTokensPerChunk: 30000);
    ...
    test('chunking decision is correct for large chat', () {
      ...
      final lines = List.generate(5000, (i) =>
      ...
      final chunks = TokenCalculator.splitIntoChunks(largeChat, maxTokensPerChunk: 30000);
      expect(chunks.length, greaterThan(1));

      // Verify no content lost
      for (int i = 0; i < lines.length; i++) {
        final found = chunks.any((c) => c.contains('message number $i'));
        expect(found, isTrue, reason: 'Message $i should be in some chunk');
      }
    });

    test('PaymentNotifier demo mode works without normalizedText', () async {
      final notifier = PaymentNotifier();
      final result = await notifier.initiateDemo(existingConversationRef: 'test-ref');
  ...
  group('InputHash web parity', () {
    test('produces valid SHA-256 for typical chat export', () {
      final chatExport = List.generate(100, (i) =>
        '[${i + 1}/05/2024, 10:00] User$i: Message content $i'
      ).join('\n');
