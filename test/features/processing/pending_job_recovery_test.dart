...
void main() {
  ...
  group('PendingJobRecoveryNotifier.refresh classification', () {
    test('returns resumable when local fields complete and server says mid-flight',
        () async {
    ...
    test('marks legacy stale rows as cancel only without probing the server',
        () async {
    ...
    test('deletes pending row when server reports payment_status=completed && email_sent=true',
        () async {
      await StorageService.instance.savePendingJobRecord(_resumableJob());
      fakeApi.onGetJobStatus = (ref) async => {
            'status': 'ok',
    ...
    test('hides Resume (shows serverFinalizing) when server says completed but email not yet sent',
        () async {
      await StorageService.instance.savePendingJobRecord(_resumableJob());
      fakeApi.onGetJobStatus = (ref) async => {
            'status': 'ok',
    ...
    test('treats HTTP 404 as cancel only (server has no record of the job)',
        () async {
      await StorageService.instance.savePendingJobRecord(_resumableJob());
      fakeApi.onGetJobStatus = (ref) async {
        throw ApiException(
    ...
    test('falls back to resumable when probe network-fails (5xx or no connection)',
        () async {
      await StorageService.instance.savePendingJobRecord(_resumableJob());
      fakeApi.onGetJobStatus = (ref) async {
  ...
  group('PendingJobRecoveryNotifier.cancel', () {
    test('deletes the pending row and drops it from state', () async {
      await StorageService.instance.savePendingJobRecord(_resumableJob());
      fakeApi.onGetJobStatus = (ref) async => {
            'status': 'ok',
            'data': {'payment_status': 'requires_capture', 'email_sent': false},
