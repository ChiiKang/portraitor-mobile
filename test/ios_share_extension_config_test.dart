...
void main() {
  group('iOS share extension configuration', () {
    test('registers Portraitor as a WhatsApp chat export share target', () {
      ...
      expect(runnerInfo, contains('<key>CFBundleDisplayName</key>'));
      expect(runnerInfo, contains('<string>Portraitor</string>'));
      expect(runnerInfo, contains('<key>CFBundleName</key>'));
      expect(runnerInfo, isNot(contains('<string>portraitor_mobile</string>')));
      expect(runnerInfo, contains(r'ShareMedia-$(PRODUCT_BUNDLE_IDENTIFIER)'));
      expect(runnerInfo, contains('<key>LSApplicationQueriesSchemes</key>'));
      expect(runnerInfo, contains('<string>whatsapp</string>'));
      ...
      expect(
        shareControllerText,
        isNot(contains('import receive_sharing_intent')),
      );
      ...
      expect(
        runnerEntitlementsText,
        contains('com.apple.security.application-groups'),
      );
      expect(
        runnerEntitlementsText,
        contains('group.ai.portraitor.portraitorMobile'),
      );
      expect(
        shareEntitlementsText,
        contains('com.apple.security.application-groups'),
      );
      expect(
        shareEntitlementsText,
      ...
      expect(
        project,
        contains('INFOPLIST_FILE = "Share Extension/Info.plist"'),
      );
      expect(
        project,
        contains(
          'PRODUCT_BUNDLE_IDENTIFIER = ai.portraitor.portraitorMobile.ShareExtension',
        ),
      );
      expect(
        project,
        contains('CUSTOM_GROUP_ID = group.ai.portraitor.portraitorMobile'),
      );
