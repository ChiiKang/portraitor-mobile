...
class ChatNormalizer {
  ...
  // Telegram HTML
  static final _telegramHtmlDoctype = RegExp(r'<!DOCTYPE\s+html', caseSensitive: false);
  ...
  static ChatFormat detectFormat(String text) {
    ...
    if (_telegramHtmlDoctype.hasMatch(text) && _telegramHtmlMessageClass.hasMatch(text)) {
    ...
    for (final line in lines) {
      ...
      checked++;
      if (_waBracket.hasMatch(trimmed) || _waUnbracket.hasMatch(trimmed) || _waIso.hasMatch(trimmed)) {
        waMatches++;
      }
    }
    ...
    for (final line in lines) {
      ...
      if (_dateHeader.hasMatch(t)) {
        ...
        timeLines++;
      } else if (t.isNotEmpty && t.length < 30
          && !RegExp(r'^\d').hasMatch(t) && !RegExp(r'^https?:', caseSensitive: false).hasMatch(t)
          && !_mediaType.hasMatch(t) && !_mediaMeta.hasMatch(t)
          && !_reply.hasMatch(t)) {
        nameLines++;
      }
    }
  ...
  static List<String> detectNames(String text) {
    ...
    final sorted = nameCounts.keys.toList()
  ...
  static String _normalizeTelegramHtml(String text) {
    ...
    while (i < tokens.length) {
      if (tokens[i].type != 'date') { i++; continue; }

      // Parse timestamp: "DD.MM.YYYY HH:MM:SS UTC+HH:00"
      final tsMatch = RegExp(r'^(\d{2})\.(\d{2})\.(\d{4})\s+(\d{2}):(\d{2}):(\d{2})')
          .firstMatch(tokens[i].value);
      if (tsMatch == null) { i++; continue; }
  ...
  static String _normalizeTelegramText(String text) {
    ...
    for (int p = 0; p < lines.length - 3; p++) {
      ...
      if (_initials.hasMatch(pInit) && pEmpty.isEmpty && _timeOnly.hasMatch(pTime)
          && pName.isNotEmpty && pName.length < 30
          && !_mediaType.hasMatch(pName) && !_reply.hasMatch(pName)
          && !_timeOnly.hasMatch(pName) && _parseDateHeader(pName) == null
    ...
    void emit(String msgText) {
      if (currentDate != null && currentTime != null && currentSender != null && msgText.isNotEmpty) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (collectingMedia) {
        if (_mediaMeta.hasMatch(trimmed) || _mediaDims.hasMatch(trimmed)
            || _mediaDur.hasMatch(trimmed) || _mediaSize.hasMatch(trimmed)) {
          prevLineEmpty = false;
          continue;
        }
      ...
      final fwdMatch = RegExp(r'^(.+?)\s+(\d{2})\.(\d{2})\.(\d{4})\s+(\d{2}):(\d{2}):(\d{2})$')
          .firstMatch(trimmed);
      if (fwdMatch != null && sawInitials) {
        currentDate = '${fwdMatch.group(2)}/${fwdMatch.group(3)}/${fwdMatch.group(4)}';
      ...
      if (sawInitials && currentTime != null) {
        final expectedName = lastInitials != null ? initialsMap[lastInitials] : null;
