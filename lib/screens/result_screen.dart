...
class _ResultScreenState extends ConsumerState<ResultScreen> {
  ...
  Future<void> _loadPortrait() async {
    final data = await StorageService.instance.getConversationById(widget.conversationId);
  ...
  List<_Section> _parseSections(String markdown) {
    ...
    if (sections.isEmpty && markdown.isNotEmpty) {
      sections.add(_Section(
        title: 'Portrait',
        summary: markdown.length > 100 ? '${markdown.substring(0, 100)}...' : markdown,
        detail: markdown,
  ...
  IconData _iconForSection(String title) {
    ...
    if (lower.contains('blind') || lower.contains('weakness')) return Icons.visibility_off_outlined;
    if (lower.contains('care') || lower.contains('love')) return Icons.favorite_outline;
    if (lower.contains('drive') || lower.contains('motiv')) return Icons.bolt_outlined;
  ...
  void _share() {
    ...
    Share.share('$name\'s Portrait by Portraitor\n\n$content');
  }
          ...
          Expanded(
            child: GhostButton(
              ...
              child: const Row(
                ...
                children: [
                  Icon(Icons.picture_as_pdf_outlined, size: 18, color: PortraitorTokens.ink),
                  SizedBox(width: 8),
                  Text('PDF'),
                ],
              ),
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Icon(widget.section.icon, size: 18, color: PortraitorTokens.brandPurple),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(widget.section.title, style: PortraitorTokens.titleSm),
