...
/// emits a `demo.v1.` Google purchase token and posts it to the real
/// `/api/google/purchase/verify.php`, which writes a genuine authorized payment
/// row, so the queue admits a real generation run and the portrait is really
/// produced and emailed. Nothing here touches Stripe, so the backend's web
/// payment mode is irrelevant to it.
///
/// Requires the backend to run with `GOOGLE_PLAY_DEMO_GRANTS` on, which
/// production never does. Without it the endpoint falls through to real Google
/// Play verification and the app reports that the purchase could not be
/// verified.
