...
class _PaymentScreenState extends ConsumerState<PaymentScreen> {
  ...
  Future<void> _pay() async {
    ...
    final isDemoMode =
        const String.fromEnvironment('DEMO_MODE', defaultValue: 'false') ==
            'true';
    if (config?.paymentAvailable == false && !isDemoMode) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Payment is currently unavailable')),
      );
    ...
    if (email.isEmpty || !email.contains('@')) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please enter a valid email to receive your portrait'),
        ),
      );
      setState(() => _isProcessing = false);
      return;
    ...
    if (success) {
      ...
      context.pushReplacement(
        ...
        extra: {
          ...
          'targetName': widget.targetName,
          'conversationId':
              paymentState.clientConversationRef ?? widget.conversationId ?? '',
  ...
  Widget build(BuildContext context) {
    ...
    final config = configAsync.valueOrNull;
    final priceDisplay = config?.amountDisplay ?? r'$5.00';
    final paymentAvailable = config?.paymentAvailable ?? true;
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      GradientButton(
                        onPressed:
                            _isProcessing || !paymentAvailable ? null : _pay,
      ...
      children: [
        Text(
          ...
          style: TextStyle(
            fontSize: fontSize,
            color:
                isSmall ? PortraitorTokens.inkMuted : PortraitorTokens.inkSoft,
        ...
        Text(
          ...
          style: TextStyle(
            fontSize: fontSize,
            color:
                isBold ? PortraitorTokens.inkStrong : PortraitorTokens.inkMuted,
